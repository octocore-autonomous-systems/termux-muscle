...
Management self-update must preserve the runtime state bytes. Test that equal targets exit 2 and older targets exit 3 with a notice before installer download, while successful newer updates and forced compatible reinstalls exit 0. Installer and integrity failures exit 1; a failed installer must not reuse either no-update code. Forced requests below 0.2.0 must still fail before invoking the old installer. Do not confuse that guard with direct execution of an old installer, which is unsupported and can bypass it, or with the supported previous-runtime `rollback` command.
