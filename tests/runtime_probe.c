...
int main(int argc, char **argv) {
    ...
    if (strcmp(name, "proot") == 0) {
        for (int i = 1; i < argc; ++i) hex("arg", argv[i]);
        const char *names[] = {"LD_PRELOAD", "LD_LIBRARY_PATH", "DISABLE_AUTOUPDATER",
                               "USE_BUILTIN_RIPGREP", "TM_CUSTOM_TEST", "TMPDIR", "BUN_TMPDIR",
                               "SSL_CERT_FILE", "SHELL", "PATH"};
        for (size_t i = 0; i < sizeof(names) / sizeof(names[0]); ++i) {
            const char *value = getenv(names[i]);
            if (value) hex(names[i], value);
        }
        ...
        if (getenv("TM_PROBE_SIGNAL")) raise(atoi(getenv("TM_PROBE_SIGNAL")));
    ...
    if (argc == 5 && strcmp(argv[1], "--nested") == 0) {
        ...
        if (size < 0) return 72;
        self[size] = '\0';
        char *command[] = {argv[2], "run", argv[3], argv[4], "current", "normal", "--", "--self-path", self, NULL};
        execute(command);
    }
    if (argc == 6 && strcmp(argv[1], "--nested-exact") == 0) {
        char *command[] = {argv[2], "run", argv[3], argv[4], argv[5], "probe", "--", "--native-ok", NULL};
        execute(command);
    }
    ...
    if (argc == 3 && strcmp(argv[1], "--self-path") == 0) {
        ...
        if (size < 0) return 72;
        value[size] = '\0';
        if (strcmp(value, argv[2]) != 0) return 73;
    ...
    if (argc == 8 && strcmp(argv[1], "--supervise") == 0) {
        ...
        if (child == 0) {
            ...
            char *command[] = {argv[2], "run", argv[3], argv[4], argv[5], "normal", "--", "--hold", argv[6], NULL};
            execute(command);
        }
