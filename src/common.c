...
void *tm_alloc(size_t size) {
    ...
    if (!p)
        tm_die("memory", "Not enough memory to finish this operation.");
    return p;
}
char *tm_strdup(const char *s) {
    if (!s)
        tm_die("invalid_value", "Required value is missing.");
    size_t n = strlen(s);
...
char *tm_path(const char *base, const char *suffix) {
    ...
    if (base_length >= PATH_MAX || suffix_length >= PATH_MAX ||
        base_length + suffix_length + 2 >= PATH_MAX)
        tm_die("unsafe_path", "Path is missing or too long.");
...
void tm_write_all(int fd, const void *data, size_t size) {
    ...
    while (size) {
        ...
        if (n < 0 && errno == EINTR)
            continue;
        if (n <= 0)
            tm_die("write_failed", "Cannot write the complete managed file.");
        p += (size_t)n;
        size -= (size_t)n;
    }
}
void tm_sync_dir(const char *path) {
    ...
    if (fd < 0 || fsync(fd))
        tm_die("sync_failed", "Cannot durably save the directory update.");
    close(fd);
}
void tm_random_hex(char *out, size_t bytes) {
    ...
    if (bytes > sizeof buf)
        tm_die("invalid_value", "Random identifier request exceeds its limit.");
...
void tm_atomic_write(const char *path, const void *data, size_t size, mode_t mode) {
    ...
    if (!name || !name[1])
        tm_die("unsafe_path", "Managed file needs a parent directory.");
    *name++ = 0;
    int dir = open(*copy ? copy : "/", O_RDONLY | O_DIRECTORY | O_NOFOLLOW | O_CLOEXEC);
    if (dir < 0)
        tm_die("unsafe_path", "Managed parent directory is unavailable.");
    struct stat old;
    if (!fstatat(dir, name, &old, AT_SYMLINK_NOFOLLOW)) {
        ...
        tm_die("unsafe_path", "Cannot inspect managed metadata.");
    char rnd[25];
    tm_random_hex(rnd, 12);
    ...
    if (fd < 0)
        tm_die("write_failed", "Cannot stage the managed file.");
    tm_write_all(fd, data, size);
    if (fchmod(fd, mode) || fsync(fd))
        tm_die("sync_failed", "Cannot durably save the managed file.");
...
char *tm_read_file(const char *path, size_t maximum, size_t *size) {
    ...
    struct stat st;
    if (fd < 0 && errno == ELOOP)
        tm_die("unsafe_path", "Managed metadata was replaced with a symlink.");
    if (fd < 0 || fstat(fd, &st))
        tm_die("invalid_file", "A required file is unavailable.");
    if (!S_ISREG(st.st_mode))
        tm_die("unsafe_path", "Expected a regular metadata file.");
    ...
    while (done < cap) {
        ...
        if (n < 0 && errno == EINTR)
            continue;
        if (n <= 0)
            tm_die("invalid_file", "A required file changed or was truncated while reading.");
        done += (size_t)n;
    }
    char extra;
    ssize_t tail;
    ...
    if (tail != 0)
        tm_die("invalid_file", "A required file grew while reading.");
    close(fd);
    if (size)
        *size = done;
    return buf;
...
void tm_digest(const char *path, const char *algorithm, unsigned char *out, unsigned int *length) {
    ...
    for (;;) {
        ...
        if (n < 0 && errno == EINTR)
            continue;
        if (n < 0)
            tm_die("integrity_failed", "Cannot finish reading the payload.");
        if (!n)
            break;
...
void tm_sha256(const char *path, char out[65]) {
    ...
    if (n != 32)
        tm_die("integrity_failed", "Unexpected digest length.");
...
bool tm_release_valid(const char *s) {
    ...
    if (!dash || !tm_hex_valid(dash + 1, 12))
        return false;
    char version[49];
...
static void finite_json(json_object *value) {
    ...
    if (json_object_is_type(value, json_type_array)) {
        for (size_t i = 0; i < json_object_array_length(value); i++)
            finite_json(json_object_array_get_idx(value, i));
...
json_object *tm_json_parse(const char *text, size_t size) {
    ...
    for (size_t i = 0; i < size; i++) {
        if (text[i] == '"')
            ...
            if (size - i >= 6 && text[i + 1] == 'u' && !memcmp(text + i + 2, "0000", 4))
                tm_die("invalid_json",
                       "Metadata strings and field names may not contain NUL bytes.");
            if (i + 1 < size)
                i++;
        }
    }
    ...
    if (!tok)
        tm_die("memory", "Cannot allocate a metadata parser.");
    ...
    while (end < size && isspace((unsigned char)text[end]))
        end++;
...
json_object *tm_json_read(const char *path) {
    size_t n;
    char *s = tm_read_file(path, TM_METADATA_MAX, &n);
    json_object *j = tm_json_parse(s, n);
    free(s);
    return j;
}
void tm_json_write(const char *path, json_object *obj) {
    ...
    size_t n = strlen(s);
    if (n >= TM_METADATA_MAX)
        tm_die("invalid_json",
               "Managed metadata including its final newline exceeds its size limit.");
    char *line = tm_alloc(n + 2);
    memcpy(line, s, n);
    line[n++] = '\n';
    tm_atomic_write(path, line, n, 0600);
    free(line);
}
...
void tm_json_print(json_object *obj) {
    puts(json_object_to_json_string_ext(obj,
