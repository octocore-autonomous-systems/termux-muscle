...
typedef struct {
    ...
    size_t size;
    int code;
    bool timeout, overflow;
} probe_result;
typedef void (*probe_callback)(void *);
static volatile sig_atomic_t interrupted_probe;
static void probe_signal(int signum) {
    interrupted_probe = signum;
}
static long long milliseconds(void) {
    struct timespec ts;
    if (clock_gettime(CLOCK_MONOTONIC, &ts))
        tm_die("clock_failed", "Cannot bound diagnostic checks.");
...
static probe_result capture(char *const argv[], const char *cwd, unsigned timeout_ms,
    ...
    struct sigaction action = {.sa_handler = probe_signal};
    sigemptyset(&action.sa_mask);
    interrupted_probe = 0;
    for (size_t i = 0; i < 3; i++)
        if (sigaction(signals[i], &action, &previous[i]))
            tm_die("probe_failed", "Cannot protect diagnostic cleanup.");
    int pipes[2];
    if (pipe2(pipes, O_CLOEXEC))
        tm_die("probe_failed", "Cannot open a diagnostic pipe.");
    fflush(NULL);
    pid_t parent = getpid(), pid = fork();
    if (pid < 0)
        tm_die("probe_failed", "Cannot start a diagnostic check.");
    if (!pid) {
    ...
    while (!finished || !eof) {
        ...
        for (;;) {
            char block[8192];
            ssize_t n = read(pipes[0], block, sizeof block);
            if (!n) {
                eof = true;
                break;
            }
            ...
            if ((size_t)n > maximum - result.size) {
                result.overflow = true;
                break;
            }
            memcpy(result.text + result.size, block, (size_t)n);
            result.size += (size_t)n;
        }
        if (!finished) {
...
static bool probe_ok(probe_result *result) {
    return !result->code && !result->timeout && !result->overflow &&
           !memchr(result->text, 0, result->size);
}
static const char *probe_detail(probe_result *result) {
    return result->timeout    ? "probe_timeout"
           : result->overflow ? "probe_output_limit"
                              : "probe_failed";
}
...
static json_object *parse_optional(const char *text, size_t size) {
    ...
    if (!valid || used != size || !json_object_is_type(value, json_type_object)) {
...
static void check(json_object *checks, const char *id, const char *status, const char *detail) {
    ...
    addstr(item, "id", id);
    addstr(item, "status", status);
    addstr(item, "detail_code", detail);
    json_object_array_add(checks, item);
}
static bool nameserver_address(char *address) {
    unsigned char binary[16];
    char *zone = strchr(address, '%');
    if (zone) {
        *zone++ = 0;
        if (!*zone || strlen(zone) > 64)
            return false;
...
static const char *resolver_configuration(const char *prefix) {
    ...
    for (const unsigned char *p = (const unsigned char *)resolved; *p; p++)
        if (*p == ':' || *p < 32 || *p == 127) {
            free(resolved);
            return "resolver_path_invalid";
        }
    int fd = open(resolved, O_RDONLY | O_NONBLOCK | O_CLOEXEC | O_NOFOLLOW);
    free(resolved);
    struct stat info;
    if (fd < 0)
        return "resolver_unavailable";
    if (fstat(fd, &info) || !S_ISREG(info.st_mode)) {
        close(fd);
        return "resolver_not_regular";
    }
    enum { maximum = 65536 };
    char buffer[maximum + 1];
    size_t size = 0;
    ...
    close(fd);
    if (size > maximum)
        return "resolver_size_limit";
    if (memchr(buffer, 0, size))
        return "resolver_contents_invalid";
    buffer[size] = 0;
    char *save = NULL;
    for (char *line = strtok_r(buffer, "\n", &save); line; line = strtok_r(NULL, "\n", &save)) {
        ...
        *end = 0;
        if (nameserver_address(address))
            return "nameserver_configured";
    }
    return "nameserver_missing_or_invalid";
}
static void resolver_probe(void *prefix) {
    puts(resolver_configuration(prefix));
}
static bool dns_check(json_object *checks, const char *prefix) {
    ...
    if (probe_ok(&result)) {
        const char *allowed[] = {"nameserver_configured",     "resolver_unavailable",
                                 "resolver_path_invalid",     "resolver_not_regular",
                                 "resolver_unreadable",       "resolver_size_limit",
                                 "resolver_contents_invalid", "nameserver_missing_or_invalid"};
        for (size_t i = 0; i < sizeof allowed / sizeof *allowed; i++)
            if (!strcmp(result.text, allowed[i]))
                detail = allowed[i];
    }
    check(checks, "dns_configuration", passed ? "PASS" : "FAIL", detail);
    free(result.text);
    return passed;
}
static json_object *environment(const char *prefix) {
    ...
    const char *properties[] = {"ro.product.manufacturer", "ro.product.model",
    ...
    for (size_t i = 0; i < sizeof keys / sizeof *keys; i++) {
        ...
        if (probe_ok(&value) && safe_text(s, 128)) {
            if (i == 3) {
                char *end;
                long number = strtol(s, &end, 10);
                json_object_object_add(
                    result, keys[i],
                    !*end && number > 0 && number < 1000 ? json_object_new_int((int)number) : NULL);
            } else
                addstr(result, keys[i], s);
        } else
            addstr(result, keys[i], NULL);
        free(value.text);
    }
    const char *manufacturer = string_value(result, "manufacturer"),
               *model = string_value(result, "model");
    if (manufacturer && model && !strcasecmp(manufacturer, "samsung") && !strcmp(model, "SM-S948U"))
        ...
        addstr(result, "device_name", NULL);
    struct utsname system;
    addstr(result, "kernel",
           !uname(&system) && safe_text(system.release, 255) ? system.release : NULL);
    ...
    addstr(result, "termux_version",
           safe_text(termux, 64) && isdigit((unsigned char)termux[0]) ? termux : NULL);
    bool source_known = source && (!strcmp(source, "GITHUB") || !strcmp(source, "F_DROID") ||
                                   !strcmp(source, "GOOGLE_PLAY_STORE"));
    ...
    for (size_t i = 0; i < sizeof names / sizeof *names; i++) {
        ...
        const char *s = trim(&value);
        addstr(packages, names[i],
               probe_ok(&value) && safe_text(s, 128) && isdigit((unsigned char)s[0]) ? s : NULL);
        free(value.text);
    }
    free(dpkg);
    json_object_object_add(result, "packages", packages);
    return result;
}
static void add_model(json_object *set, const char *id, bool *invalid) {
    if (!model_id(id) || json_object_object_length(set) > 32) {
        *invalid = true;
        return;
    }
...
static json_object *model_result(const char *requested, probe_result *probe) {
    ...
    while (begin < probe->size) {
        ...
        if (length && probe->text[begin + length - 1] == '\r')
            length--;
        ...
        if (type && !strcmp(type, "assistant")) {
            ...
            add_model(assistants, id, &invalid);
            add_model(observed, id, &invalid);
        }
        if (type && !strcmp(type, "result")) {
            finals++;
            const char *subtype = string_value(event, "subtype");
            json_object *error = NULL, *models = NULL;
            json_object_object_get_ex(event, "is_error", &error);
            success = subtype && !strcmp(subtype, "success") &&
                      json_object_is_type(error, json_type_boolean) &&
                      !json_object_get_boolean(error);
            if (json_object_object_get_ex(event, "modelUsage", &models)) {
                if (!json_object_is_type(models, json_type_object))
                    invalid = true;
                else {
                    json_object_object_foreach(models, id, value) {
                        (void)value;
                        add_model(usage, id, &invalid);
                        add_model(observed, id, &invalid);
                    }
                }
            }
        ...
        if (event)
            json_object_object_get_ex(event, "error", &error);
        const char *error_type = string_value(error, "type");
        if (error_type) {
            if (!strcmp(error_type, "authentication_error"))
                api_error = "authentication_failed";
            else if (!strcmp(error_type, "permission_error"))
                api_error = "account_permission";
            else if (!strcmp(error_type, "rate_limit_error"))
                api_error = "rate_limited";
            else if (!strcmp(error_type, "overloaded_error"))
                api_error = "service_unavailable";
            else if (!strcmp(error_type, "not_found_error"))
    ...
    if (api_error) {
        status = "FAIL";
        detail = api_error;
    } else if (!probe_ok(probe)) {
        status = "FAIL";
        detail = probe_detail(probe);
    } else if (invalid || finals > 1) {
        status = "FAIL";
        detail = "model_evidence_invalid";
    } else if (finals == 1 && !success) {
        status = "FAIL";
        detail = "model_api_error";
    } else if (fallback) {
        status = "FAIL";
        detail = "model_fallback_observed";
    } else if (finals == 1 && json_object_object_length(assistants) == 1 &&
               json_object_object_length(usage) == 1) {
        status = "PASS";
        detail = "exact_model_verified";
    }
    addstr(result, "requested", requested);
    addstr(result, "status", status);
    addstr(result, "detail_code", detail);
    json_object_object_add(result, "observed", ids);
    json_object_put(observed);
    json_object_put(assistants);
...
static void command_links_check(json_object *checks, const char *root) {
    ...
    if (value && json_object_object_get_ex(value, "entries", &entries) &&
        ...
        if (valid) {
            ...
            detail = !count  ? "no_managed_commands"
                     : owned ? "managed_commands_owned"
                             : "managed_command_changed_or_missing";
        }
    }
    check(checks, "command_links", status, detail);
    json_object_put(value);
    free(result.text);
}
static void metadata_probe(void *data) {
    ...
    char *path = tm_path(release, "payload.json");
    json_object *receipt = tm_json_read(path);
    ...
    if (!tm_version_valid(version) || !safe_text(musl, 64) || !isdigit((unsigned char)musl[0]))
        _exit(11);
...
static char *self_executable(void) {
    ...
    if (size < 1 || size >= (ssize_t)sizeof path - 1)
        tm_die("probe_failed", "Cannot locate the diagnostic helper.");
    path[size] = 0;
    return tm_strdup(path);
}
static probe_result runtime_probe(const char *self, const char *root, const char *prefix,
                                  const char *id, const char *cwd, char *const args[],
                                  unsigned timeout) {
    char *argv[64];
    size_t n = 0;
    argv[n++] = (char *)self;
    ...
    for (size_t i = 0; args[i]; i++) {
        if (n >= 63)
            tm_die("usage", "Too many diagnostic arguments.");
        argv[n++] = args[i];
    }
    argv[n] = NULL;
...
static json_object *collect_report(const char *root, const char *prefix, const char *source,
    ...
    (void)source;
    json_object *report = json_object_new_object(), *project = json_object_new_object();
    json_object *client = json_object_new_object(), *checks = json_object_new_array(),
                *model_results = json_object_new_array();
    json_object_object_add(report, "schema", json_object_new_int(1));
    char now[32];
    tm_now(now);
    addstr(report, "generated_at", now);
    addstr(project, "name", "Termux Muscle");
    addstr(project, "version", TM_VERSION);
    ...
    json_object_object_add(report, "models", model_results);
    ...
    json_object *receipt =
        probe_ok(&metadata) ? parse_optional(metadata.text, metadata.size) : NULL;
    const char *id = string_value(receipt, "id"), *version = string_value(receipt, "version"),
               *musl = string_value(receipt, "musl_version");
    bool valid =
        id && tm_release_valid(id) && version && tm_version_valid(version) && safe_text(musl, 64);
    check(checks, "runtime_integrity", valid ? "PASS" : "FAIL",
          valid ? "receipt_hashes_match" : "runtime_unavailable_or_invalid");
    char *self = self_executable();
    char *temporary = tm_path(prefix, "tmp/.termux-muscle-report-XXXXXXXX");
    bool temporary_ready = mkdtemp(temporary) != NULL,
         local_ready = valid && temporary_ready && dns_ready, model_flags_ready = false;
    if (valid) {
    ...
    for (size_t i = 0; i < sizeof lifecycle / sizeof *lifecycle; i++)
        check(checks, lifecycle[i], "SKIP", "requires_isolated_lifecycle_test");
    ...
    for (size_t i = 0; i < model_count; i++) {
        if (!local_ready || !model_flags_ready) {
            json_object *value = json_object_new_object();
            addstr(value, "requested", models[i]);
            addstr(value, "status", "SKIP");
            addstr(value, "detail_code",
                   !local_ready ? "runtime_unavailable" : "safe_model_flags_unavailable");
            json_object_object_add(value, "observed", json_object_new_array());
            json_object_array_add(model_results, value);
            continue;
        }
        char *args[] = {"-p",
                        "Reply with OK.",
...
static report_parent open_report_parent(const char *path) {
    ...
    char *resolved = realpath(copy, NULL);
    free(copy);
    if (!resolved)
        tm_die("report_publish_failed", "The report directory is unavailable.");
    int fd = open(resolved, O_RDONLY | O_DIRECTORY | O_NOFOLLOW | O_CLOEXEC);
    free(resolved);
    if (fd < 0)
        tm_die("report_publish_failed", "Cannot open the report directory.");
...
static int publish_report(const char *temporary, const char *destination) {
    ...
    int fd = openat(source.fd, source.leaf, O_RDONLY | O_NONBLOCK | O_CLOEXEC | O_NOFOLLOW);
    struct stat opened, named;
    if (fd < 0 || fstat(fd, &opened) || !S_ISREG(opened.st_mode) || opened.st_uid != getuid() ||
        fstatat(source.fd, source.leaf, &named, AT_SYMLINK_NOFOLLOW) ||
        named.st_dev != opened.st_dev || named.st_ino != opened.st_ino || fchmod(fd, 0600) ||
        fstat(fd, &opened) || (opened.st_mode & 0777) != 0600 || fsync(fd))
        tm_die("report_publish_failed",
               "Cannot secure the completed report; choose a directory supporting private files.");
    ...
    if (fsync(source.fd) || fsync(target.fd))
        tm_die("report_publish_failed",
               "The report directory does not support durable publication.");
#ifdef SYS_renameat2
    if (syscall(SYS_renameat2, source.fd, source.leaf, target.fd, target.leaf, 1U))
        tm_die("report_publish_failed",
               "Report output must be a new file on a filesystem supporting atomic publication.");
#else
    tm_die("report_publish_failed", "This system does not support atomic report publication.");
#endif
    if (fsync(target.fd) || fsync(source.fd))
        tm_die("report_publish_failed",
               "The report was published but its directory could not be synced.");
    close(fd);
    close(source.fd);
    close(target.fd);
    free(source.leaf);
...
int tm_report_main(int argc, char **argv) {
    ...
    if (argc < 4)
        tm_die("usage", "report ROOT PREFIX SOURCE [--model exact-model-id ...]");
    char *models[8];
    size_t count = 0;
    for (int i = 4; i < argc; i += 2) {
        if (!strcmp(argv[0], "doctor") || strcmp(argv[i], "--model") || i + 1 >= argc ||
            count == 8 || !model_id(argv[i + 1]))
            tm_die(
                "invalid_model",
                "Only test accepts up to eight explicit claude-* model IDs; model checks can use account credits.");
        for (size_t j = 0; j < count; j++)
            if (!strcmp(models[j], argv[i + 1]))
                tm_die("invalid_model", "Choose each model only once.");
        models[count++] = argv[i + 1];
    }
    json_object *report = collect_report(argv[1], argv[2], argv[3], count, models);
    tm_json_print(report);
    bool failed = false;
    const char *arrays[] = {"checks", "models"};
    for (size_t i = 0; i < 2; i++) {
        json_object *values;
        json_object_object_get_ex(report, arrays[i], &values);
