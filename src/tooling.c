...
static json_object *tool_record(const char *root, const char *id,
                                char **directory) {
  ...
  if (strcmp(tm_json_string(j, "id"), id) ||
      strcmp(tm_json_string(j, "phase"), "complete"))
    tm_die("tooling_identity",
           "Tool version ownership is incomplete or does not match.");
...
static void tool_verify(const char *root, const char *id) {
  ...
  if (json_object_object_length(files) < 4 ||
      json_object_object_length(files) > 1024)
    tm_die("tooling_integrity",
           "Tool file inventory is incomplete or excessive.");
  json_object_object_foreach(files, name, entry) {
    ...
    if (lstat(p, &st) || strcmp(actual, tm_json_string(entry, "sha256")) ||
        (st.st_mode & 0777) != (mode_t)json_object_get_int(
                                   tm_json_field(entry, "mode", json_type_int)))
      tm_die(
          "tooling_integrity",
...
static char *current_pointer(const char *root, bool validate_target) {
  ...
  if (lstat(p, &st) || !S_ISLNK(st.st_mode) || st.st_uid != getuid())
    tm_die("tooling_pointer",
           "Current tooling pointer was replaced; preserved.");
  char buf[128];
  ...
  if (!tm_release_valid(buf))
    tm_die("tooling_pointer",
           "Current tooling pointer must name a relative owned version.");
  if (validate_target)
...
static int lease_open(const char *root, const char *id, bool exclusive,
                      bool nonblock) {
  char *dir;
  ...
  if (flock(fd, (exclusive ? LOCK_EX : LOCK_SH) | (nonblock ? LOCK_NB : 0))) {
    ...
    if (nonblock && (error == EAGAIN || error == EWOULDBLOCK))
      return -1;
    tm_die("tooling_lease",
           "Cannot protect tool files from concurrent maintenance.");
  }
...
static char *wrapper(const char *root, const char *prefix, bool claude) {
  ...
  if (stat(bash, &shell_stat) || !S_ISREG(shell_stat.st_mode) ||
      access(bash, R_OK | X_OK) || strpbrk(bash, " \t\r\n"))
    tm_die("tooling_shell", "Termux Bash must have an executable absolute path "
                            "without whitespace.");
  char *dispatch = tm_path(root, "bin/.tm-dispatch"),
       *quoted = shell_quote(dispatch), *qr = shell_quote(root);
...
static void copy_tree(const char *source, const char *destination,
                      const char *relative, json_object *files, size_t *total,
                      unsigned int depth) {
  if (depth > 16)
  ...
  while ((e = readdir(dir))) {
    ...
    char *src = tm_path(source, e->d_name),
         *dst = tm_path(destination, e->d_name),
         *name =
             *relative ? tm_path(relative, e->d_name) : tm_strdup(e->d_name);
    struct stat st;
    ...
    if (S_ISDIR(st.st_mode)) {
      ...
      *total += (size_t)st.st_size;
      mode_t mode = (!strncmp(name, "bin/", 4) || !strncmp(name, "libexec/", 8))
                        ? 0700
                        : 0600;
...
static void stable_write(const char *root, const char *name, const char *source,
                         const char *contents, json_object *before,
                         json_object *after) {
...
static void set_current(const char *root, const char *id) {
  ...
  if (symlink(id, temporary) || rename(temporary, p))
    tm_die("tooling_publish",
           "Cannot atomically select the tested tool version.");
  tm_sync_dir(dir);
  free(dir);
  free(p);
  free(temporary);
...
static void recover_publish(const char *root) {
  ...
  const char *id = tm_json_string(journal, "id"),
             *old = tm_json_optional_string(journal, "old");
  ...
  if ((current && strcmp(current, id) && (!old || strcmp(current, old))) ||
      (!current && old))
    tm_die(
        "tooling_conflict",
  ...
  char *cli = wrapper(root, prefix, false),
       *claude = wrapper(root, prefix, true),
  ...
  if (unlink(path))
    tm_die("tooling_publish",
           "Tool selection succeeded but its journal needs recovery.");
  tm_sync_dir(root);
  free(current);
  free(dir);
  free(core);
...
static void publish(const char *root, const char *stage, const char *version,
                    const char *prefix) {
  ...
  if (present(pending_uninstall)) {
    ...
    if (!strncmp(stage_path, root, strlen(root)) &&
        stage_path[strlen(root)] == '/')
      tm_die(
          "tooling_stage",
  ...
  char *canonical = tm_canonical(stage, false),
       *package = tm_canonical(prefix, false);
  ...
  if (strcmp(text, version))
    tm_die("tooling_version",
           "Staged VERSION does not match the selected tool release.");
  free(text);
  ...
  const char *required[] = {
      "bin/termux-muscle", "libexec/tm-core", "compatibility.json",
  ...
  json_object *state = tool_state(root),
              *before = json_object_get(
                  tm_json_field(state, "stable", json_type_object)),
              *after = json_object_new_object();
  char *cli = wrapper(root, package, false),
       *claude = wrapper(root, package, true), hash[65];
  ...
  for (size_t i = 0; i < json_object_array_length(history); i++)
    json_object_array_add(
        updated, json_object_get(json_object_array_get_idx(history, i)));
...
static void run_tool(const char *root, const char *selector, int argc,
                     char **args) {
  char *id =
      !strcmp(selector, "current") ? current_tool(root) : tm_strdup(selector);
  if (!id)
    tm_die("tooling_missing",
           "No tooling release is active; rerun the source installer.");
  int fd = lease_open(root, id, false, false);
  tool_verify(root, id);
  char *dir;
  json_object *owned = tool_record(root, id, &dir);
  const char *prefix = tm_json_string(owned, "prefix");
  char *bash = tm_path(prefix, "bin/bash"),
       *script = tm_path(dir, "bin/termux-muscle");
  tm_regular(script);
  if (access(bash, X_OK))
    tm_die("tooling_shell",
           "Termux Bash is unavailable at the recorded package prefix.");
  int inherited = fcntl(fd, F_DUPFD, 10);
  if (inherited < 0)
    tm_die("tooling_lease",
           "Cannot preserve the tooling lease across execution.");
  close(fd);
  char number[32];
...
static void remove_children(int fd, unsigned int depth) {
  if (depth > 32)
    tm_die("tooling_cleanup",
           "Owned directory nesting is excessive; preserved.");
  ...
  while ((e = readdir(dir))) {
    ...
    if (fstatat(fd, e->d_name, &st, AT_SYMLINK_NOFOLLOW) ||
        st.st_uid != getuid())
      tm_die("tooling_cleanup",
             "Deletion entry changed or has a foreign owner.");
    int flag = 0;
    if (S_ISDIR(st.st_mode)) {
      int child = openat(fd, e->d_name,
                         O_RDONLY | O_DIRECTORY | O_NOFOLLOW | O_CLOEXEC);
      struct stat opened;
      if (child < 0 || fstat(child, &opened) || opened.st_dev != st.st_dev ||
          opened.st_ino != st.st_ino)
        tm_die("tooling_cleanup",
               "Deletion directory was replaced; preserved.");
    ...
    if (unlinkat(fd, e->d_name, flag))
      tm_die("tooling_cleanup",
             "Deletion interrupted; rerun uninstall or cleanup.");
  }
  closedir(dir);
  if (fsync(fd))
    tm_die("tooling_cleanup", "Cannot save deletion progress.");
}
static void recover_delete(const char *root, const int *guards,
                           size_t guard_count) {
  ...
  identity_check(root, j);
  const char *area = tm_json_string(j, "area"),
             *name = tm_json_string(j, "name"),
             *tomb = tm_json_string(j, "tomb");
  if ((strcmp(area, "tools") && strcmp(area, "releases") &&
       strcmp(area, ".")) ||
      !relative_safe(name) || strchr(name, '/') || !relative_safe(tomb) ||
      strchr(tomb, '/') || strncmp(tomb, ".tool-delete-", 13) ||
      !tm_hex_valid(tomb + 13, 24) ||
  ...
  dev_t device =
      (dev_t)json_object_get_int64(tm_json_field(j, "device", json_type_int));
  ino_t inode =
      (ino_t)json_object_get_int64(tm_json_field(j, "inode", json_type_int));
  struct stat st;
  ...
  if (!present(target) && present(original)) {
    ...
    if (strcmp(area, ".") && tm_release_valid(name)) {
      ...
      if (lstat(lease_path, &lease_stat) || !S_ISREG(lease_stat.st_mode) ||
        ...
        tm_die("tooling_lease",
               "Deletion recovery lease is missing or unsafe.");
  ...
  if (present(target)) {
    if (lstat(target, &st) || !S_ISDIR(st.st_mode) || st.st_uid != getuid() ||
      ...
      tm_die("tooling_conflict",
             "A deletion tombstone was replaced; preserved.");
    int fd = open(target, O_RDONLY | O_DIRECTORY | O_NOFOLLOW | O_CLOEXEC);
    struct stat opened;
    if (fd < 0 || fstat(fd, &opened) || opened.st_dev != device ||
        opened.st_ino != inode)
    ...
    if (rmdir(target))
      tm_die("tooling_cleanup",
             "Cannot finish deletion of the owned directory.");
    tm_sync_dir(base);
  }
...
static void delete_directory(const char *root, const char *area,
                             const char *name, const int *guards,
                             size_t guard_count) {
  ...
  json_object_object_add(j, "device",
                         json_object_new_int64((int64_t)st.st_dev));
...
static void remove_owned_drafts(const char *root) {
  ...
  while ((entry = readdir(dir))) {
    if (strncmp(entry->d_name, ".creating-", 10) ||
        !tm_release_valid(entry->d_name + 10))
      continue;
    char *path = tm_path(base, entry->d_name),
         *marker = tm_path(path, ".owned.json");
...
static void clean_tools(const char *root) {
  ...
  while ((e = readdir(dir))) {
    if (!tm_release_valid(e->d_name) ||
        (current && !strcmp(e->d_name, current)) ||
  ...
  for (size_t i = 0; i < json_object_array_length(history); i++) {
    const char *id =
        json_object_get_string(json_object_array_get_idx(history, i));
    char *p = tm_path(base, id);
    if (present(p))
...
static int own_tool_fd(const char *root, const char *id) {
  ...
  if (!text || !saved_root || !saved_id || strcmp(root, saved_root) ||
      strcmp(id, saved_id))
    return -1;
  char *end;
  errno = 0;
  ...
  struct stat a, b;
  if (fstat((int)number, &a) || lstat(p, &b) || !S_ISREG(a.st_mode) ||
      a.st_uid != getuid() || a.st_dev != b.st_dev || a.st_ino != b.st_ino)
    tm_die("tooling_lease",
           "Inherited tooling lease no longer matches its owned version.");
  free(dir);
  free(p);
  return (int)number;
}
static json_object *hold_all_leases(const char *root, int **fds,
                                    size_t *count) {
...
static void invoke_silently(int (*operation)(int, char **), int argc,
                            char **argv) {
  fflush(stdout);
...
static void uninstall(const char *root, bool keep_root) {
  ...
  if (lstat(root, &original_root) || !S_ISDIR(original_root.st_mode) ||
    ...
    tm_die("tooling_identity",
           "Installation root is unavailable or was replaced.");
  int *leases;
  size_t count;
  ...
  if (present(pending_path)) {
    ...
    if (saved_id && !tm_release_valid(saved_id))
      tm_die("tooling_journal",
             "Removal journal has an invalid current version.");
  ...
  if (present(link_path)) {
    ...
    remaining = (size_t)json_object_object_length(
        tm_json_field(links, "entries", json_type_object));
    retain = remaining > 0;
  ...
  if (retain) {
    const char *note =
        "Claude Code and owned tooling were removed. Unfamiliar files or "
    ...
    *slash = 0;
    ...
    if (lstat(root, &current_root) ||
        current_root.st_dev != original_root.st_dev ||
    ...
    const char *metadata[] = {
        "installation.json", "state.json", "tooling.json",
