...
static bool exists(const char *p) {
    ...
    if (!lstat(p, &s))
        return true;
    if (errno != ENOENT)
        tm_die("unsafe_path", "Cannot inspect the managed path.");
    return false;
}
...
static void parents(const char *p) {
    ...
    if (!slash || slash == copy)
        tm_die("unsafe_root", "Installation parent is unavailable.");
    *slash = 0;
    parents(copy);
    free(copy);
    tm_directory(p, true);
...
char *tm_store_root(const char *path) {
    ...
    char *root = tm_canonical(clean, true);
    free(clean);
    ...
    if (depth < 3 || (home && !strcmp(home, root)) || (prefix && !strcmp(prefix, root)))
        tm_die("unsafe_root",
               "Choose a dedicated installation directory, not a home or system directory.");
    free(home);
    free(prefix);
    return root;
}
json_object *tm_store_identity(const char *root) {
    ...
    json_object *j = tm_json_read(path);
    free(path);
    if (json_object_get_int(tm_json_field(j, "schema", json_type_int)) != 1 ||
        ...
        tm_die("invalid_state",
               "Installation identity is invalid or belongs to another application.");
    return j;
}
...
static void nullable_release(json_object *j, const char *key) {
    ...
    if (!json_object_object_get_ex(j, key, &v))
        tm_die("invalid_state", "Release state is incomplete.");
...
json_object *tm_store_state(const char *root) {
    char *path = tm_path(root, "state.json");
    json_object *j = tm_json_read(path);
    free(path);
    ...
    nullable_release(j, "current");
    nullable_release(j, "previous");
    json_object *history = tm_json_field(j, "history", json_type_array);
    if (json_object_array_length(history) > 10000)
        tm_die("invalid_state", "Release history exceeds its limit.");
    for (size_t i = 0; i < json_object_array_length(history); i++) {
        ...
        if (!json_object_is_type(v, json_type_string) ||
            ...
            tm_die("invalid_state", "Release history contains an unsafe identifier.");
    }
    json_object *d;
...
void tm_store_require_lock(const char *root) {
    ...
    int fd = (int)parsed;
    char *p = tm_path(root, ".lock");
    struct stat a, b;
    if (fstat(fd, &a) || lstat(p, &b) || !S_ISREG(a.st_mode) || !S_ISREG(b.st_mode) ||
        a.st_uid != getuid() || a.st_dev != b.st_dev || a.st_ino != b.st_ino ||
        flock(fd, LOCK_EX | LOCK_NB))
        tm_die("lock_required",
               "The inherited maintenance lock no longer matches this installation.");
    free(p);
    json_object_put(tm_store_identity(root));
}
static bool identity_draft(const char *name) {
    const char *prefix = ".installation.json-";
    size_t n = strlen(prefix);
    if (strlen(name) != n + 24 + 4 || strncmp(name, prefix, n) || strcmp(name + n + 24, ".tmp"))
        return false;
    char hex[25];
    memcpy(hex, name + n, 24);
    hex[24] = 0;
...
static void preflight(const char *root, bool create, char pending[37]) {
    ...
    free(idpath);
    if (!create)
        tm_die("not_installed", "No managed installation exists; run install first.");
    if (!exists(root))
        return;
    tm_directory(root, false);
    DIR *dir = opendir(root);
    if (!dir)
        tm_die("unsafe_root", "Cannot inspect installation directory.");
    ...
    while ((e = readdir(dir))) {
        if (!strcmp(e->d_name, ".") || !strcmp(e->d_name, ".."))
            continue;
        count++;
        if (!strcmp(e->d_name, ".lock"))
            continue;
        if (!identity_draft(e->d_name))
            tm_die("foreign_root",
                   "The chosen directory contains foreign files; they were preserved.");
        char *p = tm_path(root, e->d_name);
        tm_regular(p);
        free(p);
        drafts = true;
    }
    ...
    if (!st.st_size && !drafts && count == 1) {
        free(lock);
        return;
    }
    json_object *j = tm_json_read(lock);
    free(lock);
    if (strcmp(tm_json_string(j, "owner"), TM_OWNER) ||
        strcmp(tm_json_string(j, "phase"), "initializing") ||
        !uuid_valid(tm_json_string(j, "installation_id")))
        tm_die("foreign_root", "The chosen directory has an unknown ownership claim.");
    snprintf(pending, 37, "%s", tm_json_string(j, "installation_id"));
    json_object_put(j);
}
static bool dir_empty(const char *path) {
    DIR *dir = opendir(path);
    if (!dir)
        tm_die("unsafe_path", "Cannot inspect managed directory.");
    struct dirent *e;
    bool empty = true;
    while ((e = readdir(dir)))
        if (strcmp(e->d_name, ".") && strcmp(e->d_name, "..")) {
...
static void initialize(const char *root, const char *id) {
    ...
    if (!exists(p)) {
        ...
        setstr(j, "id", id);
        setstr(j, "owner", TM_OWNER);
        setstr(j, "created", now);
        tm_json_write(p, j);
        json_object_put(j);
    }
    free(p);
    ...
    if (!exists(p)) {
        ...
        if (!dir_empty(r))
            tm_die("invalid_state",
                   "State is missing but releases exist; preserved for manual recovery.");
        free(r);
        json_object *j = json_object_new_object();
        json_object_object_add(j, "schema", json_object_new_int(1));
        json_object_object_add(j, "generation", json_object_new_int64(0));
        setstr(j, "installation_id", id);
        setstr(j, "current", NULL);
        setstr(j, "previous", NULL);
    ...
    struct dirent *e;
    if (!dir)
        tm_die("unsafe_root", "Cannot complete initialization recovery.");
    while ((e = readdir(dir)))
        if (identity_draft(e->d_name)) {
            p = tm_path(root, e->d_name);
            tm_regular(p);
            if (unlink(p))
                tm_die("cleanup_failed", "Cannot remove interrupted initialization draft.");
            free(p);
        }
    closedir(dir);
    tm_sync_dir(root);
...
static int with_lock(int argc, char **argv) {
    if (argc < 5 || strcmp(argv[3], "--") ||
        (strcmp(argv[2], "create") && strcmp(argv[2], "existing")))
        tm_die("usage", "with-lock ROOT create|existing -- COMMAND ARGS...");
    char *root = tm_store_root(argv[1]);
    bool create = !strcmp(argv[2], "create");
    char pending[37] = {0};
    preflight(root, create, pending);
    if (create)
        parents(root);
    char *lockpath = tm_path(root, ".lock");
    int fd = open_lock(lockpath, true);
    free(lockpath);
    if (flock(fd, LOCK_EX | LOCK_NB))
        tm_die("busy", "Another maintenance operation is running; retry when it finishes.");
    /* A concurrent initializer could finish between preflight and acquiring. */
    preflight(root, create, pending);
    if (!*pending)
        new_uuid(pending);
    char *idpath = tm_path(root, "installation.json");
    bool initializing = !exists(idpath);
    free(idpath);
    json_object *record = json_object_new_object();
    char now[32];
    tm_now(now);
    setstr(record, "owner", TM_OWNER);
    setstr(record, "installation_id", pending);
    setstr(record, "phase", initializing ? "initializing" : "maintenance");
    setstr(record, "started", now);
    ...
    if (ftruncate(fd, 0) || lseek(fd, 0, SEEK_SET) < 0)
        tm_die("write_failed", "Cannot update ownership journal.");
    tm_write_all(fd, s, strlen(s));
    if (fsync(fd))
        tm_die("sync_failed", "Cannot save ownership journal.");
    json_object_put(record);
    if (create)
        initialize(root, pending);
    else
        json_object_put(tm_store_identity(root));
    int inherited = fcntl(fd, F_DUPFD, 10);
    if (inherited < 0)
        tm_die("lock_failed", "Cannot preserve maintenance lock across subprocesses.");
    close(fd);
    char text[32];
    snprintf(text, sizeof text, "%d", inherited);
    if (setenv("TM_LOCK_FD", text, 1) || setenv("TM_LOCK_ROOT", root, 1))
        tm_die("environment", "Cannot export maintenance lock.");
...
char *tm_store_release(const char *root, const char *id) {
    if (!tm_release_valid(id))
        tm_die("invalid_release", "Release identifier is invalid.");
    char *dir = tm_path(root, "releases");
    tm_directory(dir, false);
    char *release = tm_path(dir, id);
    free(dir);
    tm_directory(release, false);
    char *path = tm_path(release, ".owned.json");
    json_object *j = tm_json_read(path);
    free(path);
    identity_matches(root, j);
    if (strcmp(tm_json_string(j, "id"), id) ||
        json_object_get_int(tm_json_field(j, "schema", json_type_int)) != 1)
...
static void candidate(const char *root, const char *version) {
    if (!tm_version_valid(version))
        tm_die("invalid_version", "Choose an exact version in X.Y.Z form.");
    json_object_put(tm_store_state(root));
    char random[13], id[64];
    tm_random_hex(random, 6);
    snprintf(id, sizeof id, "%s-%s", version, random);
    char *releases = tm_path(root, "releases");
    tm_directory(releases, false);
    char staging_name[80];
    snprintf(staging_name, sizeof staging_name, ".creating-%s", id);
    char *staging = tm_path(releases, staging_name), *target = tm_path(releases, id);
    if (mkdir(staging, 0700))
        tm_die("candidate_failed", "Cannot allocate a private candidate directory.");
    json_object *j = json_object_new_object(), *identity = tm_store_identity(root);
    json_object_object_add(j, "schema", json_object_new_int(1));
    setstr(j, "installation_id", tm_json_string(identity, "id"));
    setstr(j, "id", id);
    char *p = tm_path(staging, ".owned.json");
    tm_json_write(p, j);
    free(p);
    p = tm_path(staging, ".lease");
    int fd = open_lock(p, true);
    free(p);
    if (fsync(fd))
        tm_die("sync_failed", "Cannot save candidate lease.");
    close(fd);
    tm_sync_dir(staging);
    if (exists(target) || rename(staging, target))
        tm_die("candidate_failed", "Cannot publish the candidate directory.");
...
static char *payload_version(const char *release) {
    ...
    json_object *j = tm_json_read(p);
    free(p);
    char *version = tm_strdup(tm_json_string(j, "version"));
    json_object_put(j);
    return version;
}
...
static void validate(const char *root, const char *id, const char *file) {
    ...
    json_object *j = tm_json_read(file);
    check_acceptance(release, j);
    char *p = tm_path(release, "acceptance.json");
    tm_json_write(p, j);
    free(p);
    free(release);
    json_object_put(j);
...
static void activate(const char *root, const char *id) {
    ...
    tm_store_verify(release);
    char *p = tm_path(release, "acceptance.json");
    json_object *a = tm_json_read(p);
    free(p);
    check_acceptance(release, a);
    json_object_put(a);
    char *version = payload_version(release);
    ...
    for (size_t i = 0; i < json_object_array_length(history); i++) {
        ...
        if (strcmp(old, id))
            json_object_array_add(updated, json_object_new_string(old));
    }
    setstr(state, "previous", previous);
    setstr(state, "current", id);
    ...
    if (generation == INT64_MAX)
        tm_die("invalid_state", "Release generation reached its limit.");
...
static bool selected(json_object *state, const char *id, size_t keep) {
    ...
    for (size_t i = 0; i < json_object_array_length(history) && i < keep; i++)
        if (!strcmp(json_object_get_string(json_object_array_get_idx(history, i)), id))
            return true;
    return false;
}
...
static void remove_children(int dirfd, unsigned int depth) {
    if (depth > 32)
        tm_die("cleanup_failed",
               "Owned tree exceeds the supported nesting limit; preserved for inspection.");
    int duplicate = dup(dirfd);
    if (duplicate < 0)
        tm_die("cleanup_failed", "Cannot inspect deletion directory.");
    ...
    while ((e = readdir(dir))) {
        ...
        if (fstatat(dirfd, e->d_name, &st, AT_SYMLINK_NOFOLLOW))
            tm_die("cleanup_failed", "Deletion entry changed during cleanup.");
        if (st.st_uid != getuid())
            tm_die("cleanup_failed", "Deletion directory contains a foreign-owned entry.");
        int flags = 0;
        if (S_ISDIR(st.st_mode)) {
            ...
            tm_die("cleanup_failed",
                   "Unexpected special file in release; preserved for inspection.");
        if (unlinkat(dirfd, e->d_name, flags))
            tm_die("cleanup_failed",
                   "Cleanup was interrupted; retry to recover the owned tombstone.");
    }
    closedir(dir);
    if (fsync(dirfd))
        tm_die("cleanup_failed", "Cannot save deletion progress.");
}
static void remove_history(json_object *state, const char *id) {
    ...
    for (size_t i = 0; i < json_object_array_length(history); i++) {
        ...
        if (strcmp(s, id))
            json_object_array_add(updated, json_object_new_string(s));
    }
    json_object_object_add(state, "history", updated);
}
static void recover_deletions(const char *root, json_object *state) {
    ...
    if (!json_object_object_get_ex(state, "deletions", &journal)) {
        json_object_object_add(state, "deletions", json_object_new_array());
        return;
    }
    char *releases = tm_path(root, "releases");
    tm_directory(releases, false);
    while (json_object_array_length(journal)) {
        ...
        if (!tm_release_valid(id) || selected(state, id, 0))
            tm_die("invalid_state", "Cleanup journal refers to an active or invalid release.");
        ...
        char name[80];
        int length = snprintf(name, sizeof name, ".deleting-%s", id);
        ...
        if (!exists(target) && exists(original)) {
            if (lstat(original, &st) || !S_ISDIR(st.st_mode) || st.st_dev != dev ||
                st.st_ino != ino)
                tm_die("cleanup_conflict",
                       "A journaled release was replaced; all replacement files were preserved.");
            lease = tm_store_lease(root, id, true, true);
            if (lease < 0)
                tm_die("busy", "A release scheduled for cleanup is still in use; retry later.");
            if (rename(original, target))
                tm_die("cleanup_failed", "Cannot recover the interrupted release rename.");
            tm_sync_dir(releases);
        }
        if (exists(target)) {
            if (lstat(target, &st) || !S_ISDIR(st.st_mode) || st.st_uid != getuid() ||
                ...
                tm_die("cleanup_conflict",
                       "A deletion tombstone was replaced; its contents were preserved.");
            int fd = open(target, O_RDONLY | O_DIRECTORY | O_NOFOLLOW | O_CLOEXEC);
            struct stat opened;
            if (fd < 0 || fstat(fd, &opened) || opened.st_dev != dev || opened.st_ino != ino ||
                opened.st_uid != getuid())
                tm_die("cleanup_conflict",
                       "Deletion directory changed while opening; it was preserved.");
            remove_children(fd, 0);
            close(fd);
            if (rmdir(target))
                tm_die("cleanup_failed", "Cannot remove the completed deletion directory.");
            tm_sync_dir(releases);
        }
        ...
        json_object_array_del_idx(journal, 0, 1);
        save_state(root, state);
        if (lease >= 0)
            close(lease);
        free(target);
...
static void cleanup(const char *root, size_t keep, bool dry_run) {
    if (keep < 2)
        tm_die("invalid_retention", "Keep at least the current and previous releases.");
    json_object *state = tm_store_state(root);
    if (!dry_run)
        recover_deletions(root, state);
    char *releases = tm_path(root, "releases");
    tm_directory(releases, false);
    DIR *dir = opendir(releases);
    if (!dir)
        tm_die("cleanup_failed", "Cannot inspect managed releases.");
    json_object *results = json_object_new_array();
    struct dirent *e;
    while ((e = readdir(dir))) {
        const char *id = e->d_name;
        if (!tm_release_valid(id) || selected(state, id, keep))
            continue;
        char *release = tm_store_release(root, id);
        ...
        if (!dry_run) {
            struct stat st;
            if (lstat(release, &st))
                tm_die("cleanup_failed", "Cannot journal the inactive release.");
            json_object *record = json_object_new_object(), *journal;
            if (!json_object_object_get_ex(state, "deletions", &journal)) {
                journal = json_object_new_array();
                json_object_object_add(state, "deletions", journal);
            }
            setstr(record, "id", id);
            ...
            json_object_array_add(journal, record);
            save_state(root, state);
            char name[80];
            int length = snprintf(name, sizeof name, ".deleting-%s", id);
            if (length < 0 || (size_t)length >= sizeof name)
            ...
            if (exists(target) || rename(release, target))
                tm_die("cleanup_failed", "Cannot stage the journaled release deletion.");
            tm_sync_dir(releases);
            free(target);
            recover_deletions(root, state);
        }
    ...
    closedir(dir);
    free(releases);
    tm_json_print(results);
    json_object_put(results);
    json_object_put(state);
}
int tm_state_main(int argc, char **argv) {
    if (!strcmp(argv[0], "with-lock"))
        return with_lock(argc, argv);
    if (argc < 3)
        tm_die("usage",
               "state ROOT show|current|candidate|validate|activate|rollback|cleanup|verify");
    char *root = tm_store_root(argv[1]);
    const char *cmd = argv[2];
    if (!strcmp(cmd, "show") && argc == 3) {
        json_object *j = tm_store_state(root);
        tm_json_print(j);
        json_object_put(j);
        ...
        for (size_t i = 0; i < json_object_array_length(history); i++) {
            ...
            const char *role = current && !strcmp(id, current)     ? "Current"
                               : previous && !strcmp(id, previous) ? "Previous"
                                                                   : "Retained";
        ...
        if (!json_object_array_length(history))
            puts("No active runtime. Run termux-muscle install.");
        json_object_put(j);
    } else if (!strcmp(cmd, "current") && argc == 3) {
        json_object *j = tm_store_state(root);
        const char *id = tm_json_optional_string(j, "current");
        if (!id)
            tm_die("not_installed", "No active Claude Code release; run install first.");
        puts(id);
        json_object_put(j);
    } else if (!strcmp(cmd, "verify") && argc == 4) {
        char *release = tm_store_release(root, argv[3]);
        int fd = tm_store_lease(root, argv[3], false, false);
        tm_store_verify(release);
        close(fd);
        free(release);
    } else {
        tm_store_require_lock(root);
        if (!strcmp(cmd, "assert-lock") &&
            argc == 3) { /* Verified above, before Bash performs any work. */
        } else if (!strcmp(cmd, "candidate") && argc == 4)
            candidate(root, argv[3]);
        else if (!strcmp(cmd, "validate") && argc == 5)
            validate(root, argv[3], argv[4]);
        else if (!strcmp(cmd, "activate") && argc == 4)
            activate(root, argv[3]);
        else if (!strcmp(cmd, "rollback") && argc == 3) {
            json_object *j = tm_store_state(root);
            const char *id = tm_json_optional_string(j, "previous");
            if (!id)
                tm_die("no_rollback", "No previous validated release is available.");
            activate(root, id);
            json_object_put(j);
            ...
            if (argc > 3) {
                ...
                if (errno || !*argv[3] || *end || v > 10000)
                    tm_die("invalid_retention", "Retention count is invalid.");
                keep = (size_t)v;
            }
            if (argc == 5) {
                if (strcmp(argv[4], "--dry-run"))
                    tm_die("usage", "Unknown cleanup argument.");
                dry = true;
            }
            cleanup(root, keep, dry);
        } else
            tm_die("usage", "Unknown state command or incorrect arguments.");
    }
    free(root);
    return 0;
}
